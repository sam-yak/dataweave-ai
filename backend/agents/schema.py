...
class SchemaAgent:
    ...
    def process(self, job_id: str, target_schema_id: str) -> list[dict]:
        ...
        for col in columns:
            ...
            if pattern_result and pattern_result["source"] == "pattern":
                ...
                mapping = {
                    ...
                    "transform_config": self._build_split_config(pattern_result.get("transform_type"), schema_json) if pattern_result.get("transform_type") in SPLIT_TRANSFORMS else pattern_result.get("transform_config", {}),
                    "status": "proposed",
        ...
        if needs_llm:
            ...
            for col_info in needs_llm:
                ...
                # v2: Build transform_config with actual target field names
                transform_config = self._build_split_config(transform_type, schema_json) if transform_type in SPLIT_TRANSFORMS else {}
    ...
    def _build_split_config(self, transform_type: str, schema_json: dict) -> dict:
        """
        Build transform_config with actual field names from the target schema.
 
        When a split_name transform is proposed, the Transform Agent needs to know
        which schema fields to write first_name and last_name into. If the schema
        uses 'consumer_first_name' instead of 'first_name', this method detects that.
        """
        ...
        if transform_type == "split_name":
            # Find the first_name field — look for fields containing "first" and "name"
            first_field = "first_name"  # default fallback
            last_field = "last_name"    # default fallback
        ...
        if transform_type == "split_address":
            # Find address component fields in the schema
